...
# 创建3行5列的子图布局
fig, axes = plt.subplots(len(z_columns), 5, figsize=(25, 9), subplot_kw={'projection': '3d'})
...
def sub_plot(x, y, z, ax, x_label='Learning rate', y_label='Batch size', log_scale=True):
    ...
    # 颜色条
    cbar = fig.colorbar(surf, ax=ax, shrink=0.5, aspect=12)
...
for j, z_col in enumerate(z_columns):  # 遍历每个z轴列
    for i, file in enumerate(files):  # 遍历每个文件
        ...
        # 提取x, y, z数据
        x = np.log10(data['lr'])  # 对x轴取对数
        y = np.log2(data['bs'])  # 对y轴取对数
        z = data[z_col]  # z轴原始数据
        sub_plot(x, y, z, ax)
...
# 手动调整子图间距
plt.subplots_adjust(left=0.005, right=0.995, bottom=0.04, top=1.08, wspace=0., hspace=-0.15)
plt.savefig('figure/optimize.pdf', dpi=300)
plt.show()
