...
from .elements import GradMonitor, Adjuster
from .env import *
...
class Trainer:
    ...
    def __init__(
        ...
        self._grad_monitor.associate(self.model_wrapped)
        self._adjuster.associate(self.optimizer, self._grad_monitor)
    ...
    def _inner_training_loop(
        ...
        print(">>>>>>", self._train_batch_size, self.args.gradient_accumulation_steps)
        ...
        with self.model_wrapped.no_sync():
                ...
                for _ in range(total_updates):
                    ...
                    for inputs in batch_samples:
                        ...
                        if do_sync_step:
                            ...
                            #########################################                         
                            # if dist.get_rank() == 0:
                            #     with open(f'ep/ept_{args.learning_rate}_{args.per_device_train_batch_size}_{args.gradient_accumulation_steps}_{dist.get_world_size()}.txt', 'a') as file:
                            #         file.write(f'{self._grad_monitor.ept}\n')
                            #     with open(f'ep/epb_{args.learning_rate}_{args.per_device_train_batch_size}_{args.gradient_accumulation_steps}_{dist.get_world_size()}.txt', 'a') as file:
                            #         file.write(f'{self._grad_monitor.epb}\n')
    ...
    def _load_from_checkpoint(self, resume_from_checkpoint, model=None):
        ...
        other_checkpoint = torch.load(f'./tmp_{self._job_id}/checkpoint.pth', weights_only=True)
        self._ideal_global_bs = other_checkpoint['ideal_global_bs']
        self._adjuster.set_accumulate_steps(self.args.gradient_accumulation_steps)
        self._adjuster.set_init_bs_config(other_checkpoint['max_bs'], other_checkpoint['max_global_bs'])
        self._grad_monitor.set_accumulation_steps(self.args.gradient_accumulation_steps)
    ...
    def _maybe_log_save_evaluate(self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        ...
        if self.control.should_evaluate:
            ...
            print(f"[RANK{dist.get_rank()}]: end-eval {time.time()}")  
    ...
    def save_model(self, output_dir: Optional[str] = None, _internal_call: bool = False):
        ...
        torch.save(other_checkpoint, f'./tmp_{self._job_id}/checkpoint.pth')
    ...
    def profile_max_bs(self):
        ...
        while True:
            try:
                self.args.gradient_accumulation_steps = accumu_steps
                self._train_batch_size = ideal_global_bs // accumu_steps
                train_dataloader = self.get_train_dataloader()
                sample_epb = []
                for index, data in enumerate(train_dataloader):
                    loss = self.training_step(self.model_wrapped, data)
                    self._grad_monitor.monitor_epb(False)
                    if (index + 1) % accumu_steps == 0:
                        epb = self._grad_monitor.epb
                        self.model_wrapped.zero_grad()
                        if epb is not None:
        ...
        self._adjuster.set_init_bs_config(math.ceil(ideal_global_bs / accumu_steps / 4), self._ideal_global_bs * 32)
        torch.cuda.empty_cache()
        self.adjust_resources()
    ...
    def adjust_resources(self):
        ...
        self._train_batch_size = math.ceil(self._ideal_global_bs / world_size() / self.args.gradient_accumulation_steps)
        
        if new_world_size > world_size():
            if rank() == 0:
                self._save_checkpoint(self.model_wrapped, None)
    ...
    def set_accumulation_steps(self, new_world_size: int):
        ...
        self.args.gradient_accumulation_steps = new_accumulation_steps
        self._adjuster.set_accumulate_steps(new_accumulation_steps)
